...
2. Написать функцию, которая производит поиск и выводит на экран вакансии с заработной платой больше введённой суммы. Необязательно - возможность выбрать вакансии без указанных зарплат
...
class HHscraper:
    ...
    def insert_into_mongo_db_hh(self):
        with MongoClient(MONGO_HOST, MONGO_PORT) as client:
            db = client[MONGO_DB]
            MONGO_COLLECTION = 'HH'
            vacancy = db[MONGO_COLLECTION]
            vacancy.insert_many(self.info_vacance)
...
class SJscraper:
    ...
    def insert_into_mongo_db_sj(self):
        with MongoClient(MONGO_HOST, MONGO_PORT) as client:
            db = client[MONGO_DB]
            MONGO_COLLECTION = 'SJ'
            vacancy = db[MONGO_COLLECTION]
            vacancy.insert_many(self.info_sj_vacance)
            vacancy.delete_many({'мин зарплата': {'$lt': min_salary}})
...
if __name__ == '__main__':
    ...
    # scraper_hh.insert_into_mongo_db_hh()
    ...
    scraper_sj.insert_into_mongo_db_sj()
